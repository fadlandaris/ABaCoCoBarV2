...
import { imageScan } from '@/data/data';
...
export default function ScanPage() {
  ...
  return (
    <section className="h-screen p-12 pt-24">
      {/* NAV */}
      ...
      <div className="w-full h-full flex flex-col justify-between relative">
        ...
        <div
          className="border-t-2 border-l-2 border-r-2 w-full h-full rounded-t-2xl border-dashed max-w-5xl mx-auto border-neutral-400 flex justify-center items-center
                     group hover:bg-gradient-to-b from-[#fbf9f5] via-[#f2fab8]/50 to-[#d1df5a]/50 transition-all duration-[600ms] cursor-pointer"
        >
            ...
            >
              <div className="relative">
                ...
                <div className="relative w-40 h-20 mx-auto mb-6">
                  {imageScan.map((item, i) => {
                    const positionClass = [
                      'top-1/2 -translate-y-1/2 left-2 text-fourth z-0 -rotate-24',
                      'top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 text-primary z-10',
                      'top-1/2 -translate-y-1/2 right-2 text-tertiary z-0 rotate-24',
                    ];
                    const delay = [0.0, 0.3, 0.6];
                    return (
                      <motion.div
                        key={i}
                        className={`absolute ${positionClass[item.id]}`}
                        initial={{ y: 0 }}
                        animate={{ y: [0, -20, 0] }}
                        transition={{
                          duration: 2,
